...
int main(int argc, char** argv) {
    ...
    SDL_Window* win = SDL_CreateWindow("Map & Categories UI", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINDOW_W, WINDOW_H, SDL_WINDOW_RESIZABLE);
    ...
    auto makeColor = [&]() { return Color{ (Uint8)(rand() % 156 + 80), (Uint8)(rand() % 156 + 80), (Uint8)(rand() % 156 + 80), 255 }; };
    ...
    auto worldToMap = [&](int wx, int wy)->pair<int, int> {
        int mx = wx - mapX;
        int my = wy - mapY;
    ...
    auto computeLayout = [&](int w, int h) {
        windowW = w; windowH = h;
    ...
    auto drawButton = [&](SDL_Renderer* r, TTF_Font* f, const string& text, SDL_Rect rect, Color bg, bool glow) {
        ...
        if (glow) {
            SDL_SetRenderDrawColor(r, bg.r, bg.g, bg.b, 80);
        ...
        if (f) {
            ...
            SDL_Texture* tex = createTextTexture(r, f, text, white, tw, th);
            if (tex) {
    ...
    while (running) {
        ...
        if (state == MAIN_VIEW) {
            ...
            for (size_t i = 0; i < categories.size(); ++i) {
                SDL_Rect b{ bx, by + (int)i * (bh + spacing), bw, bh };
                drawButton(ren, font, categories[i].name, b, categories[i].color, categories[i].selected);
            }

            SDL_Rect addBtn{ 20, 240, mapX - 40, 40 }; drawButton(ren, font, "Add Category", addBtn, Color{ 70,130,180,255 }, textInputActive);
            SDL_Rect removeAllBtn{ 20, 290, mapX - 40, 40 }; drawButton(ren, font, "Remove All", removeAllBtn, Color{ 180,70,70,255 }, false);
        ...
        if (state == VENDING_VIEW && selectedCategoryIndex >= 0 && selectedCategoryIndex < (int)categories.size()) {
            ...
            SDL_Rect backBtn{ bx, by, 42, 36 }; drawButton(ren, font, "<-", backBtn, Color{ 100,100,100,255 }, false);
            SDL_Rect addPointBtn{ bx + 54, by, bw - 54, bh };
            drawButton(ren, font, "Add point (click map)", addPointBtn, Color{ 80,160,80,255 }, awaitingMapClickForAddPoint);

            by += (bh + gap);

            SDL_Rect remPointBtn{ bx, by, bw, bh }; drawButton(ren, font, "Remove nearest point (click map)", remPointBtn, Color{ 200,100,80,255 }, false);

            by += (bh + gap);

            SDL_Rect searchBtn{ bx, by, bw, bh };
            drawButton(ren, font, "Search nearest point to mouse", searchBtn, Color{ 100,140,220,255 }, lastSearchIdx != -1);

            by += (bh + gap);

            SDL_Rect deleteCatBtn{ bx, by, bw, bh }; drawButton(ren, font, "Delete Category", deleteCatBtn, Color{ 180,60,60,255 }, false);
